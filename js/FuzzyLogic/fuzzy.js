...
function draw_variable(v, i) {
	...
	for (var j=0;j<v.sets.length;j+=1) {
		document.getElementById("variable_"+i+"_info").innerHTML +=v.setsName[j]+": "+v.sets[j]+"<br>";
		//draw_function(v.sets[j], i, 400 / (max - min), colors[j]);
		draw_function(v.sets[j], i, 150 / (max - min), colors[j]);
	}
}
...
function draw_One(i){
	...
	var colors = ["#f00","#0f0","#00f","#f0f","#0ff","#ff0","#999"];
	var v = obj.variables_input[i];
	var aux = "<div id='variable_"+i+"'>"+v.name+"<table><tr><td id='variable_"+i+"_rap'></td><td id='variable_"+i+"_info'></td></tr></table></div>";
	document.getElementById("fuzzy").innerHTML += aux;
...
function draw_path(s, i, ratio, color) {
	return paper[i].path("M"+parseInt(ratio*s[0])+" 70L"+parseInt(ratio*s[1])+" 0L"+parseInt(ratio*s[2])+" 0L"+parseInt(ratio*s[3])+" 70");
}*/
